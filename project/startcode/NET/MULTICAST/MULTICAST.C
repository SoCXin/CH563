...
*                      (3)、本程序用于演示多播通讯，单片机收到数据后，回传给远端，ARM966,Thumb,小端
*******************************************************************************/
...
#define     R32_ETHE_MAHT0               (*((PUINT32V)0X00406010))
#define     R32_ETHE_MAHT1               (*((PUINT32V)0X00406014))
#define     R32_ETHE_MACCR               (*((PUINT32V)0X00406088))
#define     RX_BROADPKT                  1<<17
#define     RX_MULTI_EN                 1<<9
...
/* CH563相关定义 */
const UINT8 MACAddr[6] = {0x02,0x03,0x04,0x05,0x06,0x09};                       /* CH563MAC地址 */
const UINT8 IPAddr[4] = {192,168,1,19};                                          /* CH563IP地址 */
...
UINT8  MulticastIP[4] = {234,5,6,7};                                 /* 多播IP地址 */
UINT8 MultiMac[6]  = {0x01,0,0x5e,0x40,0x10,0x01};                               /* 对应多播Mac地址 */
/*******************************************************************************
...
int main( void ) 
{
    ...
    else
    {
      Hash0 |= ((UINT32)1<<MacCrc32);
		...
		printf("Hash0:%lx  ",(UINT32)Hash0);
    printf("Hash1:%lx  \n",(UINT32)Hash1);
		R32_ETHE_MAHT0  = Hash0;                                                //将多播的MAC地址填入Hash表
		R32_ETHE_MAHT1  = Hash1;
		R32_ETHE_MACCR |= (RX_BROADPKT|RX_MULTI_EN);				                    //设置MAC过滤方式为允许广播+Hash
		R32_ETHE_MACCR |= 1<<12;	
